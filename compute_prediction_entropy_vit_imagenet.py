...
from utils.imagenet_c import set_imagenet_corruptions_data_handlers, ImageNet_C
...
def _run_for_checkpoint(args):
    ...
    train_loader = get_imagenet_vit(
        datadir=datadir, train=True, bs=64,
        transform=imagenet_preprocessing_training_set,
    )
    ...
    for ratio in compression_ratios:
        ...
        if ratio == 0.0:
            ...
            if args.method not in ("taylor", "hessian"):
                ...
                compressed_model = one_shot_structural_pruning_vit(
        ...
        # Sparsity (no REPAIR for ViT, which uses LayerNorm)
        ...
        for corruption in corruptions:
            corr_loader = imagenet_c_dataset.get_corruption_data_loader(
    ...
    out_path = os.path.join(
        out_dir,
        f"prediction_entropy_vit_imagenet_{args.method}_sev{args.severity}{seed_suffix}.json",
    )
...
def main(args):
    ...
    if hasattr(args, 'checkpoints') and args.checkpoints:
        ...
        checkpoint_paths = [getattr(args, 'checkpoint', None)]
    ...
    if len(checkpoint_paths) > 1 and len(all_results) > 1:
        ...
        print(f"\nvit_imagenet_{args.method}_multi_ckpt_prediction_entropy_mean = {{")
        for ratio_str in all_ratios:
            print(f"  '{ratio_str}': {agg_mean[ratio_str]},")
        print("}")

        print(f"\nvit_imagenet_{args.method}_multi_ckpt_prediction_entropy_std = {{")
        for ratio_str in all_ratios:
        ...
        agg_path = os.path.join(
            out_dir,
            f"prediction_entropy_vit_imagenet_{args.method}_sev{args.severity}_multi_ckpt.json",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute prediction-level softmax entropy for compressed ViT-Base on ImageNet-C"
    )
    ...
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="Path to checkpoint file (if None, uses timm pretrained)")
    parser.add_argument("--checkpoints", type=str, default=None,
                        help="Comma-separated list of checkpoint paths for multi-seed")
    parser.add_argument("--seed_tag", type=str, default="",
                        help="Optional seed tag appended to the output JSON filename "
                             "so multi-seed runs save per-seed files. Auto-set in "
                             "multi-checkpoint mode (0/1/2/...) unless provided.")
    parser.add_argument("--corruptions_root", type=str,
                        default="{}/Few-Shot-Adaptation-Learning/datasets/imagenet-c/{}/{}")
    parser.add_argument("--dataset_root", type=str,
                        default="{}/REDS-Resource-Efficient-Deep-Subnetworks/datasets/imagenet")
    parser.add_argument("--encoder_name", type=str, default="vit_base_patch16_224")
