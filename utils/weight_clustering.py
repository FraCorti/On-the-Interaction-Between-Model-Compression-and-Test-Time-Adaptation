...
class WeightClustering:
    def __init__(self, n_clusters, n_features, normalize=False, use_kmeans=True, device=None):
    ...
    def __call__(self, weight):
        ...
        if self.use_kmeans is True:
            if HKMeans is not None:
                km = HKMeans(n_clusters=self.n_clusters, random_state=None, n_init=10,
                             n_jobs=-1, max_iter=20, verbose=False)
            else:
                km = KMeans(n_clusters=self.n_clusters, random_state=None, n_init=10, max_iter=20)
...
def get_average_correlation(merge, w, device=None):
    ...
    avg_corr = torch.tensor(avg_corr).to(device).float()
...
def merge_channel_clustering(perm_to_axes, params, p_name, merge, custom_merger=None, device=None):
...
def merge_channel_clustering_approx_repair(perm_to_axes, params, p_name, merge, custom_merger=None, device=None):
    params = copy.deepcopy(params)
    n = params[perm_to_axes[p_name][0][0]].shape[perm_to_axes[p_name][0][1]]
    w = concat_weights(perm_to_axes, params, p_name, n)

    avg_corr = get_average_correlation(merge=merge, w=w.detach().cpu().numpy())
...
def compress_weight_clustering(perm_to_axes, params, max_ratio=0.5, threshold=0.1, hooks=None, approx_repair=False,
                               merge_layer=None, custom_merger=None, device=None):
    ...
    for p_name in merge_sizes.keys():
        ...
        if approx_repair is False:
            prm = merge_channel_clustering(perm_to_axes, params, p_name, merge, custom_merger=custom_merger, device=device)
        else:
            prm = merge_channel_clustering_approx_repair(perm_to_axes, params, p_name, merge,
                                                         custom_merger=custom_merger, device=device)
