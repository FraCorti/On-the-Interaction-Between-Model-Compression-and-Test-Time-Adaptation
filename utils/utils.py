...
def spa_configure_model(model):
    """Configure model for use with SPA."""
...
def fuse_block(block, block_len, override=True):
    ...
    for i in range(block_len):
        ...
        if len(get_module_by_name(block[i], sh)) == 2:
            ...
            get_module_by_name(block[i], sh)[1].weight.data = torch.ones_like(
                get_module_by_name(block[i], sh)[1].weight.data)
            get_module_by_name(block[i], sh)[1].bias.data = torch.zeros_like(
                get_module_by_name(block[i], sh)[1].bias.data)
            ...
            get_module_by_name(block[i], sh)[1].running_mean.data = torch.zeros_like(
                get_module_by_name(block[i], sh)[1].running_mean.data)
            get_module_by_name(block[i], sh)[1].running_var.data = torch.ones_like(
                block[i].shortcut[1].running_var.data)
...
def eval_model(model, dataloader, device=None):
    ...
    return float(correct / total_num), float(total_loss / total_num)
...
def load_model(model, path, mapping=None, device=None):
    # weights_only=False is set explicitly to silence FutureWarning while maintaining backward compatibility
    # with potential pickles containing arbitrary objects.
