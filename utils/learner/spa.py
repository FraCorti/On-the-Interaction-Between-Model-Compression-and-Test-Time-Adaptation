...
@register('spa')
def spa(model, optimizer, steps=1, episodic=False):
    return SPA(model=model, optimizer=optimizer, steps=steps, episodic=episodic)


class SPA(nn.Module):
    """SPA adapts a model by entropy minimization during testing.
    Once tented, a model adapts itself by updating on every forward.
    """

    def __init__(self, model, optimizer, noise_ratio=0.4, freq_mask_ratio=0.2, use_eata=False, fishers=None,
                 fisher_alpha=2000.0, steps=1, episodic=False, e_margin=0.4 * math.log(1000), d_margin=0.05,
                 imagenet_mask=None):
        super().__init__()
        self.model = model
    ...
    def forward_and_adapt_eata(self, x):
        ...
        loss = 0
        if self.use_eata:
            loss += 0.1 * self.get_eta_loss(outputs, anti_forget=False)
    ...
    def get_eta_loss(self, outputs, anti_forget=False):
        # adapt
...
def check_model(model):
    ...
    assert not has_all_params, "tent should not update all params: " \
                               "check which require grad"
    has_bn = any([isinstance(m, (nn.BatchNorm1d, nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm)) for m in model.modules()])
