...
@register('sar')
def sar(model, optimizer, steps=1, episodic=False, margin_e0=0.4 * math.log(1000),
        reset_constant_em=0.2):
    return SAR(model=model, optimizer=optimizer, steps=steps, episodic=episodic,
               margin_e0=margin_e0, reset_constant_em=reset_constant_em)
...
def check_model(model):
    ...
    assert not has_all_params, "SAR should not update all params: " \
                               "check which require grad"
    has_norm = any([isinstance(m, (nn.BatchNorm1d, nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm)) for m in model.modules()])
