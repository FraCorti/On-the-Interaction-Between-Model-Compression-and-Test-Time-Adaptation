...
class LearnablePatchErasing(torch.nn.Module):
    ...
    def __init__(self, patch_size=16, img_size=224):
        ...
        self.alpha = nn.Parameter(torch.ones(h * w) * math.sqrt(0.4), requires_grad=True)
    ...
    def forward(self, img):
        ...
        for _, patch in enumerate(patches):
            ...
            new_img[..., patch[0]:patch[0] + self.patch_size, patch[1]:patch[1] + self.patch_size] = \
                (1 - self.alpha[_] ** 2) * img[..., patch[0]:patch[0] + self.patch_size,
                                           patch[1]:patch[1] + self.patch_size] + self.alpha[_] ** 2 * v
        new_img.clip_(-1, 1)
        return new_img
