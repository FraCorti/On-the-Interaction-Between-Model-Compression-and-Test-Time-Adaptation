...
def _run_for_checkpoint(args):
    ...
    train_loader = get_imagenet(datadir=datadir, train=True, bs=64)
    ...
    out_path = os.path.join(
        out_dir,
        f"prediction_entropy_imagenet_{args.method}_sev{args.severity}{seed_suffix}.json",
    )
...
def main(args):
    ...
    if len(checkpoint_paths) > 1 and len(all_results) > 1:
        ...
        print(f"\nresnet18_imagenet_{args.method}_multi_ckpt_prediction_entropy_mean = {{")
        for ratio_str in all_ratios:
            print(f"  '{ratio_str}': {agg_mean[ratio_str]},")
        print("}")

        print(f"\nresnet18_imagenet_{args.method}_multi_ckpt_prediction_entropy_std = {{")
        for ratio_str in all_ratios:
        ...
        agg_path = os.path.join(
            out_dir,
            f"prediction_entropy_imagenet_{args.method}_sev{args.severity}_multi_ckpt.json",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute prediction-level softmax entropy for compressed ResNet-18 on ImageNet-C"
    )
    ...
    parser.add_argument("--checkpoint", type=str,
                        default="pretrained/resnet18_imagenet.pth")
